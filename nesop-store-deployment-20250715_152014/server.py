                ...
                if ad_auth_result[0]:  # Check if authentication was successful
                    ...
                    if ad_user:
                        ...
                        if not local_user:
                            ...
                            logging.info(f"Importing new AD user to local database: {username}")
                            is_admin = ad_manager.is_user_admin(ad_user)
                            ...
                            ad_manager.log_audit_event(
                                username, 
                                'user_import', 
                                'AD user imported to local database',
                                {'is_admin': is_admin}
                            )
                        else:
                            # Update existing user's AD sync timestamp
                            db_utils.update_ad_sync_timestamp(username)
                            
                            # Log the login
                            ad_manager.log_audit_event(
                                username, 
                                'login_success', 
                                'AD user logged in successfully'
                            )
                        
                        # Get updated user info
                        user_info = db_utils.get_user(username)
                        return jsonify({
                            'success': True,
                            'user': {
...
def search_ad_users():
    ...
    try:
        ...
        for user in ad_users:
            ...
            user['is_admin'] = ad_manager.is_user_admin(user)
