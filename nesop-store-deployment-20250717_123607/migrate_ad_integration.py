...
def get_db_connection():
    ...
    try:
        # Use the current value of db_utils.DB_PATH (not a cached copy)
        conn = sqlite3.connect(db_utils.DB_PATH)
        return conn
    except Exception as e:
        logger.error(f"Failed to connect to database at {db_utils.DB_PATH}: {str(e)}")
        raise
...
def backup_database():
    ...
    try:
        ...
        if not os.path.exists(db_utils.DB_PATH):
            logger.info(f"Database file does not exist yet: {db_utils.DB_PATH}")
            return None
        
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        backup_path = f"{db_utils.DB_PATH}.backup_{timestamp}"
        
        # Copy the database file
        with open(db_utils.DB_PATH, 'rb') as src:
...
def add_ad_columns_to_users():
    ...
    if not table_exists('users'):
        logger.info("Users table does not exist yet - skipping AD column addition")
        logger.info("Note: Base tables should be created before running this migration")
        return
...
def main():
    ...
    try:
        ...
        if backup_path:
            ...
            logger.info("No existing database to backup - proceeding with fresh setup")
